...
class Actor(Model):
    ...
    def call(self, state):
        ...
        action_asli = action *2

        # Calculate the log probability
        log_pi_ = dist.log_prob(action_)
        # Change log probability to account for tanh squashing as mentioned in
        # Appendix C of the paper
        log_pi = log_pi_ - tf.reduce_sum(tf.math.log(2*(1 - action**2) + EPSILON), axis=1,
                                         keepdims=True)

        return action_asli , log_pi
...
class SoftActorCritic:
    ...
    def update_alpha(self, current_states):
        ...
        #print("hiii", grads,variables,alpha_loss)
